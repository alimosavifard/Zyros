...
const Login = () => {
  ...
  const formik = useFormik({
	...
	onSubmit: async (values, { setSubmitting, setErrors }) => {
	  try {
		await loginUser(values); // فقط تابع را اجرا می‌کنیم تا کوکی تنظیم شود
		await login(); // سپس وضعیت احراز هویت را به روز می‌کنیم
		toast.success(t('login') + ' موفق');
		navigate('/');
	  } catch (error) {
		if (error.response?.status === 401) {
			setErrors({ password: t('error.invalidCredentials') });
		} else {
			toast.error(t('error.failedLogin'));
		}
	  } finally {
		setSubmitting(false);
	  }
